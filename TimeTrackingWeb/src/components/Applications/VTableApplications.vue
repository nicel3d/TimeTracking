...
export default class VTableApplications extends Mixins(SkipTake) {
  ...
  ExportXLSXList () {
    this.$store.state.api.applications_ExportXLSXGetListWithoutFilter(this.dataRequest)
      .then(res => DownloadingFileForBrowsers(res, filename, FileFormatEnum.XLSX))
      .catch(res => this.$root.$emit('snackbar', res))
  }

  ExportCSVList () {
    this.$store.state.api.applications_ExportCSVGetListWithoutFilter(this.dataRequest)
