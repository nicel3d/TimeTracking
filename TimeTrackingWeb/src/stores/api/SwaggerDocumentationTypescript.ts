...
export class WSApi {
  ...
  applications_ImportGetListWithoutFilter(value: TableSortingRequest | null): Promise<FileResponse | null> {
    let url_ = this.baseUrl + "/api/Applications/ImportGetListWithoutFilter";
    url_ = url_.replace(/[?&]$/, "");
    ...
    return this.http.fetch(url_, options_).then((_response: Response) => {
      return this.processApplications_ImportGetListWithoutFilter(_response);
    });
  }

  protected processApplications_ImportGetListWithoutFilter(response: Response): Promise<FileResponse | null> {
    const status = response.status;
    let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
    if (status === 200 || status === 206) {
      const contentDisposition = response.headers ? response.headers.get("content-disposition") : undefined;
